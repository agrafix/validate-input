...
main =
  hspec $
    do
      describe "conformsPred" $
        do
          it "should result in error when value does not conform pred" $
            property $ \(i :: Int) ->
      ...
      describe "minLength" $
        do
          it "should result in error when string is to small" $
      ...
      describe "maxLength" $
        do
          it "should result in error when string is to large" $
            property $ \(str :: String) (i :: Int64) ->
              fromIntegral (length str) > i
      ...
      describe "regex" $ do
        it "passes" $
          runValidator (matchesRegex "[[:lower:]]+" "lowercase") "foo" `shouldBe` Right "foo"
        it "fails" $
          runValidator (matchesRegex "[[:lower:]]+" "lowercase") "FOO" `shouldBe` Left "lowercase"
